...
def DrawGLScene():
	...
	#def s(theta, phi):
	#	return( r*math.cos(theta)*math.cos(phi), r*math.sin(theta), r*math.cos(phi)*math.sin(theta))
	...
	while phi <= (math.pi/2):
		...
		while theta <= (math.pi * 2):
			glTexCoord2f(theta/(math.pi*2), (phi+(math.pi/2))/math.pi)
			glVertex3fv(s(theta, phi))

			glTexCoord2f((theta/(math.pi*2)) + deltaX, (phi+(math.pi/2))/math.pi)
			glVertex3fv(s(theta+delta, phi))

			glTexCoord2f((theta/(math.pi*2)) + deltaX, (phi+(math.pi/2))/math.pi + deltaY)
			glVertex3fv(s(theta+delta, phi+delta))

			glTexCoord2f(theta/(math.pi*2), (phi+(math.pi/2))/math.pi + deltaY)
